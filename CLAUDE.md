...
> A nested `CLAUDE.md` in `apps/api/src/modules/` covers how to add a feature
> module — it loads automatically when you work in that folder. (Add per-app
> `apps/*/CLAUDE.md` later only if an app grows rules that don't fit here.)
...
- **New feature** → a new `modules/<name>/` folder, then mount its router in
  ...
  what types the web client). Details in `apps/api/src/modules/CLAUDE.md`.
